...
function CommentCard({ data, boardId, idx }: { data: Comment; boardId: number; idx: number }) {
  ...
  const { isLoggedIn, userId, nickname, bootcampName } = useSelector((state: RootState) => state.auth);
  ...
  const handlerEditConfirmBtn = () => {
    ...
    axios.put(`${process.env.REACT_APP_API_URL}/comments/` + data.id, requestEdit).then((res) => {
      dispatch(modifyComment({ idx, content: res.data.data.content, anonymous: res.data.data.anonymous }));
      setActiveInputType(NORMAL);
    });
    return;
  };
  ...
  const handlerReplySubmitBtn = (input: string) => {
    ...
    axios.post(`${process.env.REACT_APP_API_URL}/comments`, replyComment).then((res) => {
      if (res.data.message === 'success') {
        setActiveInputType(NORMAL);
        axios.get(`${process.env.REACT_APP_API_URL}/comments/list/` + boardId).then((res) => {
            ...
            <CommentWriter>
              ...
              <div style={{ display: 'flex', gap: '15px' }}>
                ...
                {isLoggedIn && data.isWriter && activeInputType === NORMAL && (
                  <ModeEditOutlineOutlinedIcon onClick={handlerSetEditBtn} sx={{ color: colors.TEXT_LIGHT }} />
                )}
                {isLoggedIn && data.isWriter && activeInputType === NORMAL && (
                  <CloseOutlinedIcon onClick={handlerSetDelBtn} sx={{ color: colors.TEXT_LIGHT }} />
                )}
                {data.isWriter && activeInputType === EDIT && (
                  <CloseOutlinedIcon onClick={handlerCancelEditBtn} sx={{ color: colors.TEXT_LIGHT }} />
                )}
          ...
          <CommentCardContentsArea>
            ...
            <StyledCommentCard>
              ...
              <ReplyInput
                ...
                handlerAnonymousBtn={handlerClickAnonymous}
                handlerConfirmBtn={handlerReplySubmitBtn}></ReplyInput>
            </StyledCommentCard>
          </CommentCardContentsArea>
        </WrapperStyledCommentCard>
      )}
