...
function BoardListPage() {
  ...
  useEffect(() => {
    if (sort === 0) {
      axios.get(API_URL + `/${selectCategory}`).then((res) => setBoardListData(res.data.data.content));
      return;
    }
    let sortType = '';
    if (sort === 1) {
    ...
    axios.get(API_URL + `/${selectCategory}/${sortType}`).then((res) => setBoardListData(res.data.data.content));
  }, [selectCategory, sort]);
